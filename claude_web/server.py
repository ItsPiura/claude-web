...
AUTH_USER = os.environ.get("CLAUDE_WEB_USER", "").strip()
AUTH_PASSWORD = os.environ.get("CLAUDE_WEB_PASSWORD", "").strip()
_AUTH_ENABLED = bool(AUTH_USER and AUTH_PASSWORD)
_AUTH_SALT = "claude-web-auth-v1"
_TOKEN_EXPIRY_SECONDS = 24 * 60 * 60

_PUBLIC_PREFIXES = ("/api/auth/", "/changelog.json", "/uploads/")
_PUBLIC_EXACT = {"/", "/login"}
...
def _get_jwt_secret() -> bytes:
    return hashlib.pbkdf2_hmac("sha256", AUTH_PASSWORD.encode("utf-8"), _AUTH_SALT.encode("utf-8"), 100000, dklen=32)
...
def _create_jwt(sub: str) -> str:
    ...
    payload = base64.urlsafe_b64encode(
        json.dumps({"sub": sub, "iat": now, "exp": now + _TOKEN_EXPIRY_SECONDS}).encode()
    ).rstrip(b"=").decode()
    msg = f"{header}.{payload}".encode()
    sig = base64.urlsafe_b64encode(hmac.digest(_get_jwt_secret(), msg, hashlib.sha256)).rstrip(b"=").decode()
...
def _verify_jwt(token: str) -> Optional[str]:
    ...
    expected_sig = base64.urlsafe_b64encode(hmac.digest(_get_jwt_secret(), msg, hashlib.sha256)).rstrip(b"=").decode()
...
async def _auth_middleware(request, call_next):
    ...
    if not auth_header.startswith("Bearer "):
        return Response(
            content=json.dumps({"detail": "unauthorized"}, ensure_ascii=False),
            status_code=401,
    ...
    if username is None:
        return Response(
            content=json.dumps({"detail": "token expired or invalid"}, ensure_ascii=False),
            status_code=401,
            media_type="application/json",
        )
...
async def auth_login(req: LoginRequest):
    if not _AUTH_ENABLED:
        return {"token": "", "username": "", "auth_enabled": False}
    if not hmac.compare_digest(req.username, AUTH_USER) or not hmac.compare_digest(req.password, AUTH_PASSWORD):
        raise HTTPException(status_code=401, detail="invalid credentials")
    token = _create_jwt(req.username)
    return {"token": token, "username": AUTH_USER, "auth_enabled": True}
...
def main():
    ...
    parser.add_argument("--auth-user", type=str, default="", help="Username for basic auth (env: CLAUDE_WEB_USER)")
    parser.add_argument("--auth-password", type=str, default="", help="Password for basic auth (env: CLAUDE_WEB_PASSWORD)")
    ...
    global AUTH_USER, AUTH_PASSWORD, _AUTH_ENABLED
    if args.auth_user:
        AUTH_USER = args.auth_user.strip()
    if args.auth_password:
        AUTH_PASSWORD = args.auth_password.strip()
    _AUTH_ENABLED = bool(AUTH_USER and AUTH_PASSWORD)
    ...
    if _AUTH_ENABLED:
        ...
        print(f"  → Auth: disabled (no credentials configured)")
    ...
    if args.host not in _LOCAL_HOSTS:
        print()
        if _AUTH_ENABLED:
            print(f"  ⚠️  claude-web bound to {args.host} — auth is ENABLED.", file=sys.stderr)
            print("     Only use on a trusted network (e.g. tailscale, VPN, SSH tunnel).", file=sys.stderr)
        else:
